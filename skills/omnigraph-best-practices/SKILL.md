...
compatibility: Targets omnigraph CLI >= 0.7.0 — the unified `load`, the two config surfaces (cluster.yaml + ~/.omnigraph/config.yaml), and cluster apply/serve all require 0.7.0, which ships on the `edge` channel until it tags.
metadata:
  author: ModernRelay
  version: "0.6.0"
